...
class Device(ABC):
    ...
    def send_individual(
        ...
        if interface.is_type_properties():
            raise ValidationError(
                f"Interface {interface_name} is of type properties. You should use " "set_property."
            )
        if payload is None:
    ...
    def send_object(
        ...
        # no need to check for property type since interfaces are already checked in the constructor
        # to not have aggregation = object and type = properties
        if payload is None:
            raise ValidationError("Payload should be different from None")
    ...
    def set_property(
        ...
        if not interface.is_type_properties():
            raise ValidationError(
                f"Interface {interface_name} is not of type properties. You should use "
